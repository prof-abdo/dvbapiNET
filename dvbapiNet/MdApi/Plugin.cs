...
namespace dvbapiNet.MdApi
{
    public static class Plugin
    {
        ...
        private static IntPtr _SubMenu = IntPtr.Zero;
        private static uint _MenuItemId = 0;
        public static uint MenuItemId => _MenuItemId;
        ...
        public static void OnMenuSelect(uint dwMenuId)
        {
            dvbapiNet.DvbViewer.Plugin.Configure(dvbapiNet.DvbViewer.Plugin._DvbViewerHwnd);
        }
        ...
        [DllExport(ExportName = "SetMenuHandle", CallingConvention = CallingConvention.StdCall)]
        public static void SetMenuHandle(IntPtr hMenu)
        {
            try
            {
                if (hMenu != IntPtr.Zero)
                {
                    _MenuItemId = (uint)(40000 + (_DllId > 0 ? _DllId : 0));
                    IntPtr subMenu = CreatePopupMenu();
                    if (subMenu != IntPtr.Zero)
                    {
                        AppendMenu(subMenu, 0x0000 /* MF_STRING */, new UIntPtr(_MenuItemId), "dvbapiNet");
                        InsertMenu(hMenu, 0xFFFFFFFF, 0x0410 /* MF_BYPOSITION|MF_POPUP */, subMenu, "dvbapiNET");
                    }
                }
            }
            catch (Exception ex)
        ...
        [DllExport(ExportName = "MenuItemClick", CallingConvention = CallingConvention.StdCall)]
        public static void MenuItemClick(int itemId)
        {
            dvbapiNet.DvbViewer.Plugin.Configure(dvbapiNet.DvbViewer.Plugin._DvbViewerHwnd);
        }
