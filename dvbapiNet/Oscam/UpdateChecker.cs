...
namespace dvbapiNet.Oscam
{
    ...
    public static class UpdateChecker
    {
        ...
        {
            return IsNewerThanCurrent(tagName, Globals.PluginAssembly.GetName().Version);
        }

        /// <summary>
        /// Vergleicht einen Release-Tag gegen die lokal installierte Version.
        ...
        public static bool IsNewerThanCurrent(string tagName, Version local)
        {
            ...
            string clean = tagName.Trim().TrimStart('v', 'V').Trim();
            if (!Version.TryParse(clean, out var remote)) return false;

            // AssemblyVersion uses the "X.Y.Z.*" scheme, so Build carries the real patch level
            // while Revision is auto-generated by the compiler and must never be compared.
            // Tags without an explicit patch ("2.4") parse with Build = -1, hence the clamp.
            int rMaj = remote.Major, rMin = remote.Minor, rBld = remote.Build < 0 ? 0 : remote.Build;
            int lMaj = local.Major, lMin = local.Minor, lBld = local.Build < 0 ? 0 : local.Build;
