...
namespace dvbapiNet.Oscam
{
    ...
    public class DvbApiAdapter : IDisposable
    {
        ...
        {
            try { DecryptionMonitor.Instance.Attach(this); } catch { }
            ...
            try
            {
                ...
                // Failover: serveurs alternatifs depuis "dvbapi.servers" (CSV "host:port,host:port,...")
                string backups = "";
                Globals.Config.Get(cConfigSection, "servers", ref backups);
                if (!string.IsNullOrWhiteSpace(backups))
                {
                    foreach (var raw in backups.Split(','))
                    {
                        string s = raw.Trim();
                        ...
                        if (!int.TryParse(s.Substring(colon + 1).Trim(), out int p)) continue;
                        if (h == srv && p == port) continue; // skip primary
                        _ApiClient.AddBackupServer(h, p);
                    }
                }
